...
const KEY = join(process.env.TEMP!, "_nx_od")
...
// Deploy both src and scripts to dev
scp(["nx-overdue.tar.gz", `${HOST}:/tmp/nx-overdue.tar.gz`])
scp(["nx-overdue-scripts.tar.gz", `${HOST}:/tmp/nx-overdue-scripts.tar.gz`])

run(["systemctl", "stop", "nexus-dev"])
run(["rm", "-rf", "/opt/nexus-dev/src"])
run(["mkdir", "-p", "/opt/nexus-dev/src"])
run(["tar", "-xzf", "/tmp/nx-overdue.tar.gz", "-C", "/opt/nexus-dev/src"])
run(["rm", "-rf", "/opt/nexus-dev/scripts"])
run(["mkdir", "-p", "/opt/nexus-dev/scripts"])
run(["tar", "-xzf", "/tmp/nx-overdue-scripts.tar.gz", "-C", "/opt/nexus-dev/scripts"])
run(["rm", "/tmp/nx-overdue.tar.gz", "/tmp/nx-overdue-scripts.tar.gz"])
