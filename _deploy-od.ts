...
const KEY = join(process.env.TEMP!, "_nx_od2")
...
scp(["nx-od.tar.gz", `${HOST}:/tmp/nx-od.tar.gz`])
scp(["nx-od-scripts.tar.gz", `${HOST}:/tmp/nx-od-scripts.tar.gz`])
run(["systemctl", "stop", "nexus-dev"])
run(["rm", "-rf", "/opt/nexus-dev/src"])
run(["mkdir", "-p", "/opt/nexus-dev/src"])
run(["tar", "-xzf", "/tmp/nx-od.tar.gz", "-C", "/opt/nexus-dev/src"])
run(["rm", "-rf", "/opt/nexus-dev/scripts"])
run(["mkdir", "-p", "/opt/nexus-dev/scripts"])
run(["tar", "-xzf", "/tmp/nx-od-scripts.tar.gz", "-C", "/opt/nexus-dev/scripts"])
run(["rm", "/tmp/nx-od.tar.gz", "/tmp/nx-od-scripts.tar.gz"])
