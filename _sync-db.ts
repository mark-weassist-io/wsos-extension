...
const KEY = join(process.env.TEMP!, "_nx_sync")
...
console.log("=== Copying DB to VPS ===")
scp(["data\\wsos-extension.db", `${HOST}:/tmp/wsos-extension.db`])

console.log("=== Dev ===")
ssh("systemctl stop nexus-dev && cp /opt/nexus-dev/data/wsos-extension.db /opt/nexus-dev/data/wsos-extension.db.bak && cp /tmp/wsos-extension.db /opt/nexus-dev/data/wsos-extension.db && systemctl start nexus-dev && sleep 3 && systemctl is-active nexus-dev")

console.log("=== Prod ===")
ssh("systemctl stop nexus && cp /opt/nexus/data/wsos-extension.db /opt/nexus/data/wsos-extension.db.bak && cp /tmp/wsos-extension.db /opt/nexus/data/wsos-extension.db && systemctl start nexus && sleep 3 && systemctl is-active nexus")
