...
const KEY = join(process.env.TEMP!, "_nx_rv")
...
scp(["nx-revert.tar.gz", `${HOST}:/tmp/nx-revert.tar.gz`])
run(["systemctl", "stop", "nexus-dev"])
run(["rm", "-rf", "/opt/nexus-dev/src"])
run(["mkdir", "-p", "/opt/nexus-dev/src"])
run(["tar", "-xzf", "/tmp/nx-revert.tar.gz", "-C", "/opt/nexus-dev/src"])
run(["rm", "/tmp/nx-revert.tar.gz"])
