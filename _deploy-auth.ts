...
const KEY = join(process.env.TEMP!, "_nx_au")
...
scp(["nx-auth.tar.gz", `${HOST}:/tmp/nx-auth.tar.gz`])
run(["systemctl", "stop", "nexus-dev"])
run(["rm", "-rf", "/opt/nexus-dev/src"])
run(["mkdir", "-p", "/opt/nexus-dev/src"])
run(["tar", "-xzf", "/tmp/nx-auth.tar.gz", "-C", "/opt/nexus-dev/src"])
run(["rm", "/tmp/nx-auth.tar.gz"])
