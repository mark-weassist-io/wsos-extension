...
const KEY = join(process.env.TEMP!, "_nx_wr")
...
scp(["nx-wrap.tar.gz", `${HOST}:/tmp/nx-wrap.tar.gz`])
run(["systemctl", "stop", "nexus-dev"])
run(["rm", "-rf", "/opt/nexus-dev/src"])
run(["mkdir", "-p", "/opt/nexus-dev/src"])
run(["tar", "-xzf", "/tmp/nx-wrap.tar.gz", "-C", "/opt/nexus-dev/src"])
run(["rm", "/tmp/nx-wrap.tar.gz"])
